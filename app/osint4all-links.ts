export type OsintDirectoryEntry = {
  id: string;
  category: string;
  name: string;
  url: string;
...
export const OSINT4ALL_SOURCE_URL = "https://start.me/p/L1rEYQ/osint4all";
export const OSINT4ALL_CAPTURED_AT = "2026-07-28";

export const OSINT4ALL_LINKS = [
  {
...
] as const satisfies readonly OsintDirectoryEntry[];

export const OSINT4ALL_CATEGORIES = ["THROWAWAY CONTACT","ID GENERATOR","UNIFIED SEARCH","PEOPLE","USERNAME","EMAIL","PHONE","SOCIAL MEDIA","FACEBOOK","TWITTER","SEARCH ENGINES","GOOGLE CSE","MAPS","GEO"] as const;

export const OSINT4ALL_CATEGORY_DESCRIPTIONS: Readonly<Record<string, string>> = {
  "THROWAWAY CONTACT": "Temporary email, SMS, telephone, fax, VoIP, or privacy-preserving contact resource.",
  "ID GENERATOR": "Synthetic identity-data generator or identifier-format reference. Generated data is not a real identity or evidence.",
  "UNIFIED SEARCH": "Cross-source OSINT search, enrichment, or investigation framework.",
  "PEOPLE": "Public-record, directory, family-history, or people-discovery resource.",
  "USERNAME": "Username discovery, availability, and cross-platform account-correlation resource.",
  "EMAIL": "Email discovery, validation, header analysis, reputation, or authorized exposure-research resource.",
  "PHONE": "Telephone numbering, carrier, reverse-lookup, messaging, or communications research resource.",
  "SOCIAL MEDIA": "Cross-platform social-media discovery and SOCMINT research resource.",
  "FACEBOOK": "Facebook-focused public-content, account, page, group, or identifier research resource.",
  "TWITTER": "X/Twitter-focused public-post, account, network, archive, or analytics resource.",
  "SEARCH ENGINES": "General, specialist, privacy-oriented, document, vulnerability, or dataset search resource.",
  "GOOGLE CSE": "Purpose-built Google Custom Search Engine for a specific public-data or platform corpus.",
  "MAPS": "Mapping, live-event, transport, satellite, hazard, imagery, or geographic visualization resource.",
  "GEO": "Geospatial intelligence, gazetteer, catalog, dataset, API, or analysis resource."
};
...
export function describeOsintDirectoryEntry(entry: OsintDirectoryEntry) {
  ...
  const category = OSINT4ALL_CATEGORY_DESCRIPTIONS[entry.category] ?? "Externally curated OSINT resource.";
  return `${category} OSINT4All lists this entry as “${entry.name}”; hosted at ${osintDirectoryHost(entry)}.`;
}

const DIRECTORY_QUERY_ALIASES: Readonly<Record<string, readonly string[]>> = {
  geolocation: ["geo", "geospatial", "maps", "mapping"],
  location: ["geo", "geospatial", "maps", "mapping"],
  aircraft: ["flight", "aviation", "maps"],
  vessel: ["ship", "maritime", "maps"],
  person: ["people", "public-record"],
};

export function searchOsintDirectory(query: string, limit = 50) {
  const words = query.toLowerCase().match(/[a-z0-9.-]{2,}/g)?.filter((word) => !["the", "and", "for", "with", "tool", "tools", "search", "find"].includes(word)) ?? [];
  const terms = [...new Set(words.flatMap((word) => [word, ...(DIRECTORY_QUERY_ALIASES[word] ?? [])]))];
  if (!terms.length) return OSINT4ALL_LINKS.slice(0, Math.max(1, Math.min(limit, 100)));
  return OSINT4ALL_LINKS.map((entry) => {
    const haystack = `${entry.name} ${entry.category} ${entry.url} ${describeOsintDirectoryEntry(entry)}`.toLowerCase();
    const score = terms.reduce((total, term) => total + (haystack.includes(term) ? (entry.name.toLowerCase().includes(term) || entry.category.toLowerCase().includes(term) ? 3 : 1) : 0), 0);
    return { entry, score };
  }).filter(({ score }) => score > 0).sort((left, right) => right.score - left.score || left.entry.id.localeCompare(right.entry.id)).slice(0, Math.max(1, Math.min(limit, 100))).map(({ entry }) => entry);
}
