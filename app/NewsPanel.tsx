...
export function NewsPanel() {
  ...
  return <section className="phase-panel news-panel">
    ...
    <div className="news-layout"><aside><header><span>RSS SOURCES</span><b>{enabled.length} / {sources.length} ARMED</b></header>{sources.map((source) => { const health = status.find(({ id }) => id === source.id); return <label key={source.id} className={enabled.includes(source.id) ? "enabled" : ""}><input type="checkbox" checked={enabled.includes(source.id)} onChange={() => toggle(source.id)} /><span><strong>{source.name}</strong><small>{source.description}</small><em>{health?.error ? health.error : health?.lastSuccessfulFetchAt ? `LAST ${new Date(health.lastSuccessfulFetchAt).toLocaleTimeString()} // ${health.cachedItems} ITEMS` : `MINIMUM ${Math.round(source.minimumCadenceMs / 60_000)} MIN`}</em></span></label>;})}<p>No feed is contacted until you press Pull. Responses are capped at 1 MB, requests run two at a time, conditional caching is used, and repeated manual pulls are held for 30 seconds.</p></aside><main>{visible.length ? visible.map((item) => <article key={item.id}><header><span>{item.sourceName}</span><time>{new Date(item.publishedAt ?? item.retrievedAt).toLocaleString()}</time></header><a href={item.url} target="_blank" rel="noreferrer">{item.title}</a>{item.summary && <p>{item.summary}</p>}</article>) : <div className="panel-empty"><span>NO CACHED HEADLINES</span><strong>EXTERNAL TRAFFIC IS IDLE</strong><p>Select sources and pull them when you want news data to leave and return to this computer.</p></div>}</main><aside className="news-awareness"><header><span>OSINT4ALL LIVE SOURCES</span><a href={OSINT4ALL_SOURCE_URL} target="_blank" rel="noreferrer">SOURCE BOARD ↗</a></header><p>These current-event and live-awareness tools were identified in the captured OSINT4ALL directory. They open directly and are not scraped in the background.</p>{awarenessLinks.map((entry) => <a key={entry.id} href={entry.url} target="_blank" rel="noreferrer"><strong>{entry.name}</strong><small>{describeOsintDirectoryEntry(entry)}</small></a>)}</aside></div>
  </section>;
}
