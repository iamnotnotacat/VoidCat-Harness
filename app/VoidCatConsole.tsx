...
import type { DiagnosticsSnapshot } from "./DiagnosticsPanel";
...
import type { DocumentRecord, RegisteredFolderRecord } from "./RagPanel";
...
import { createDefaultHunterWorkspace } from "../build/hunter-seeker/source-workspace";
...
const RagPanel = lazy(() => import("./RagPanel").then(({ RagPanel }) => ({ default: RagPanel })));
const WebPanel = lazy(() => import("./WebPanel").then(({ WebPanel }) => ({ default: WebPanel })));
const OsintProviderPanel = lazy(() => import("./OsintProviderPanel").then(({ OsintProviderPanel }) => ({ default: OsintProviderPanel })));
const OsintDirectoryPanel = lazy(() => import("./OsintDirectoryPanel").then(({ OsintDirectoryPanel }) => ({ default: OsintDirectoryPanel })));
const ProjectsPanel = lazy(() => import("./ProjectsPanel").then(({ ProjectsPanel }) => ({ default: ProjectsPanel })));
const NewsPanel = lazy(() => import("./NewsPanel").then(({ NewsPanel }) => ({ default: NewsPanel })));
const AppSettingsPanel = lazy(() => import("./AppSettingsPanel").then(({ AppSettingsPanel }) => ({ default: AppSettingsPanel })));
const UnitSettingsPanel = lazy(() => import("./UnitSettingsPanel").then(({ UnitSettingsPanel }) => ({ default: UnitSettingsPanel })));
const SupportVcPanel = lazy(() => import("./SupportVcPanel").then(({ SupportVcPanel }) => ({ default: SupportVcPanel })));
...
export function VoidCatConsole() {
  ...
  const [hunterSetupRequested, setHunterSetupRequested] = useState(false);
  ...
  const openHunterSetup = useCallback(() => {
    setHunterSetupRequested(true);
    setView("hunter");
  }, []);

  const handleHunterSetupRequest = useCallback(() => setHunterSetupRequested(false), []);
  ...
  useEffect(() => {
    ...
    const refreshFolderStatus = async () => {
      try {
        const response = await fetch("/api/rag/folders/status", { cache: "no-store" });
        if (!response.ok) return;
        const data = await response.json() as { folders?: RegisteredFolderRecord[] };
        if (!active || !data.folders) return;
        setPersistent((current) => ({ ...current, ragFolders: data.folders ?? current.ragFolders }));
        if (!data.folders.some((folder) => folder.status === "queued" || folder.status === "scanning")) await refreshPersistentState();
      } catch { /* the next lightweight status poll may recover */ }
    };
    const timer = window.setInterval(() => { void refreshFolderStatus(); }, 1_000);
  ...
  return <main className={`console fx-${animationLevel} ${booted ? "is-booted" : ""}`}>
    ...
    <section className={`command-grid view-${view}`}>
      <aside className="rail"><p className="rail-code">SYS.05</p><nav aria-label="Primary navigation"><button className={view === "hunter" ? "active" : ""} onClick={() => setView("hunter")}><span>01</span> HUNTER-SEEKER</button><button className={view === "news" ? "active" : ""} onClick={() => setView("news")}><span>02</span> NEWS WATCH</button><button className={view === "chat" ? "active" : ""} onClick={() => setView("chat")}><span>03</span> COMMAND</button><button className={view === "models" ? "active" : ""} onClick={() => setView("models")}><span>04</span> UNIT BANK</button><button className={view === "osint-directory" ? "active" : ""} onClick={() => setView("osint-directory")}><span>05</span> OSINT DIRECTORY</button><button className={view === "osint" ? "active" : ""} onClick={() => setView("osint")}><span>06</span> OSINT PROVIDERS</button><button className={view === "projects" ? "active" : ""} onClick={() => setView("projects")}><span>07</span> PROJECTS</button><button className={view === "library" ? "active" : ""} onClick={() => setView("library")}><span>08</span> RAG LIBRARY</button><button className={view === "archive" ? "active" : ""} onClick={() => setView("archive")}><span>09</span> ARCHIVE</button><button className={view === "profiles" ? "active" : ""} onClick={() => setView("profiles")}><span>10</span> PROFILES</button><button className={view === "memory" ? "active" : ""} onClick={() => setView("memory")}><span>11</span> MEMORY</button><button className={view === "web" ? "active" : ""} onClick={() => setView("web")}><span>12</span> WEB ACCESS</button><button className={view === "diagnostics" ? "active" : ""} onClick={() => setView("diagnostics")}><span>13</span> DIAGNOSTICS</button><button className={view === "app-settings" ? "active" : ""} onClick={() => setView("app-settings")}><span>14</span> APP SETTINGS</button><button className={view === "unit-settings" ? "active" : ""} onClick={() => setView("unit-settings")}><span>15</span> UNIT SETTINGS</button><button className={view === "support-vc" ? "active" : ""} onClick={() => setView("support-vc")}><span>16</span> SUPPORT_VC</button></nav><div className="rail-status"><span>PHASE</span><strong>06</strong><p>VOICE +<br />DISTRIBUTION</p></div></aside>
