...
export function HunterSeekerSetupGuide({ step, maritimeCredentialSaved, onStep, onClose, onComplete, onConfigureMaritime, onRemoveMaritime }: {
  step: number;
  ...
  return <div className="hunter-setup-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="hunter-setup-dialog" role="dialog" aria-modal="true" aria-labelledby="hunter-setup-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="hunter-setup-content">
        ...
        {currentStep === 1 && <><p className="hunter-setup-lead">A free aisstream.io account unlocks live vessel positions for one selected region. The API key is encrypted by Windows and never returns to this screen.</p><article className="hunter-setup-access"><b>AIS</b><div><span>FREE ACCOUNT REQUIRED</span><strong>AISSTREAM.IO MARITIME</strong><small>{maritimeCredentialSaved === null ? "CHECKING PROTECTED STORAGE" : maritimeCredentialSaved ? "CREDENTIAL STORED" : "NO CREDENTIAL"}</small></div><a href="https://aisstream.io/authenticate" target="_blank" rel="noreferrer">OPEN OFFICIAL SIGN-IN ↗</a></article><div className="hunter-setup-inline-actions"><button className="primary-action" onClick={onConfigureMaritime}>{maritimeCredentialSaved ? "EDIT KEY / REGION" : "ADD API KEY"}</button>{maritimeCredentialSaved && <button className="danger-action" onClick={() => void onRemoveMaritime()}>REMOVE SAVED KEY</button>}</div></>}
        ...
        {currentStep === 4 && <><p className="hunter-setup-lead">The live board is ready. Setup can be reopened at any time from the SETUP button on the Situation Board.</p><div className="hunter-setup-summary"><article><span>PUBLIC SOURCES</span><strong>4 READY</strong></article><article><span>MARITIME</span><strong>{maritimeCredentialSaved ? "READY" : "SKIPPED"}</strong></article><article><span>RETENTION</span><strong>MEMORY ONLY</strong></article></div></>}
      </div>

      <footer><button className="cancel-action" disabled={currentStep === 0} onClick={() => void onStep(currentStep - 1)}>BACK</button><button className="local-only-action" onClick={() => void advance()}>{currentStep === 4 ? "FINISH" : "SKIP FOR NOW"}</button>{currentStep < 4 && <button className="primary-action" onClick={() => void advance()}>CONTINUE</button>}</footer>
    </section>
  </div>;
}
