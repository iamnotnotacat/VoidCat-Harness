...
test("the desktop launch chain, authenticated health check, visible-window gate, and clean shutdown remain wired", async () => {
  ...
  assert.match(launcher, /release\\VoidCat Harness-win32-x64\\VoidCat Harness\.exe/); assert.match(launcher, /node_modules\\electron\\dist\\electron\.exe/); assert.match(desktop, /await ensureLocalService\(\)/); assert.match(desktop, /mainWindow\.once\("ready-to-show"/); assert.match(desktop, /app\.on\("before-quit"/); assert.match(desktop, /event\.preventDefault\(\)/); assert.match(desktop, /cleanupLocalResources\(\)\.finally/);
