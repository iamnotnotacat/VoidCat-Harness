...
test("the Windows package prepares a checksum-pinned bundled Whisper runtime with custom overrides remaining optional", () => {
  ...
  assert.match(packageJson, /prepare:voice/); assert.match(packageJson, /npm run prepare:voice/); assert.match(packageScripts["package:windows"], /package-windows\.mjs/); assert.doesNotMatch(packageScripts["package:windows"], /--no-asar/); assert.match(packageJson, /update-windows-launcher\.ps1/); assert.match(prepare, /whisper-bin-x64\.zip/); assert.match(prepare, /ggml-tiny\.en-q5_1\.bin/); assert.match(prepare, /archiveSha256/); assert.match(prepare, /modelSha1/); assert.match(prepare, /requiredEngineFiles/); assert.match(prepare, /rmSync\(join\(releaseDirectory, entry\.name\)/); assert.match(pack, /asar:\s*\{\s*unpack:/); assert.match(pack, /unpackDir:\s*"\{app,build,dist,node_modules,vendor\}"/); assert.match(pack, /Package size budget exceeded/); assert.match(pack, /forbidden development content/); assert.match(launcher, /VoidCat Harness\.exe/); assert.match(main, /app\.asar\.unpacked/); assert.match(main, /localServerRoot/); assert.match(main, /bundledWhisperExecutable/); assert.match(main, /bundledWhisperModel/); assert.match(main, /"-sns"/); assert.match(main, /180_000/); assert.match(settings, /READY OUT OF BOX/); assert.match(settings, /ADVANCED OVERRIDES/);
});
...
test("external applications cannot be opened by popups, sounds, or UNIT lifecycle actions", () => {
  ...
  assert.match(preload, /bridgeVersion: 9/);
